...
export function ServiceProcess({ service }: ServiceProcessProps) {
  const steps = getServiceProcessSteps(service);
  const lineRef = useRef<HTMLDivElement>(null);
  const inView = useInView(lineRef, { once: true, amount: 0.35 });
  const isDev = service.processVariant === "development";
      ...
      <div className="container-padded relative z-10">
        <AnimateIn className="mx-auto max-w-2xl text-center">
          ...
          <p className="section-subtext mx-auto">
            {isDev
              ? "A delivery path built for product and engineering clarity."
              : "A clear four-step path from insight to impact — designed for clarity and delivery."}
          </p>
        </AnimateIn>
        ...
        <div ref={lineRef} className="relative mt-12">
          ...
          <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-4 lg:gap-6">
            {steps.map((step, i) => {
              const Icon = serviceIconMap[step.icon] ?? serviceIconMap.Search;
              return (
